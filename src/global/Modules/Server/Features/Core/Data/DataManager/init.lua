...
export type Module = typeof(DataController) & ModuleData
...
function DataController._SetupPlayerProfile(self: Module, player: Player)
...
-- [ Public Functions ] --
function DataController.AddData(self: Module, player: Player, value: number, path: string): boolean
...
function DataController.SetData(self: Module, player: Player, value: any, path: string, hardSet: boolean?): boolean
    if type(value) == "number" and (value ~= value) then
        warn("Invalid numeric value provided. Expected a number.")
        return false
    end
    ...
    if typeof(value) == "number" and value < 0 then
        warn("Attempted to set a negative value in DataController.SetData. Player:", player, "Path:", path, "Value:", value)
        return false
    end
...
function DataController.UpdateData(self: Module, player: Player, cb: (data: ProfileConfig.ProfileTemplate) -> ()): boolean
    local Profile = self:GetProfile(player)
...
function DataController.GetData(self: Module, player: Player, path: string): (boolean, any)
    local Profile = self:GetProfile(player)
...
function DataController.GetProfile(self: Module, player: Player): Profile
    ...
    if not Profile then
        error(("[DataController] Profile not ready for %s"):format(player.Name))
    end

    return Profile
end

function DataController.OnPlayerAdded(self: Module, Maid: Maid.Maid, player: Player)
    self:_SetupPlayerProfile(player)
end

function DataController.Init(self: Module, serviceBag: ServiceBag.ServiceBag)
